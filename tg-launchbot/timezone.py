...
def remove_time_zone_information(db_path: str, chat: str):
	...
	try:
		cursor.execute(
			...
			(None, None, chat))
		logging.info(
			f'✅ User successfully removed their time zone information!')

	except:
		logging.exception(
			f'❓ User tried to remove their time zone information, but ran into exception.'
		)
...
def update_time_zone_string(db_path: str, chat: str, time_zone: str):
	...
	logging.info(
		f'🌎 User successfully set their time zone locale to {time_zone}')
